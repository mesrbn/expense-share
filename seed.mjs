...
const supabase = createClient(
  ...
  // In order to create users after enabling RLS(row level security), you must substitute the supabase key with service row key in the setting -> api
...
async function seedTransactions() {
  ...
  const { error: deleteError } = await supabase
    .from("transactions")
    .delete()
    .gte("id", "5061488c-6046-4e49-9f34-84f4db374526");
